...
export default function Data({ params, data }) {
  ...
  const { data: dataTahunAjaran, isLoading, refetch } = useQuery({
    queryKey: ['dataTahunAjaran'], queryFn: async () => {
  ...
  const handleClick = (tahunAjaran) => {
    ...
    route.push(`/dashboard/sekolah/${data.nama}/${tahunAwal[0]}?semester=Semester 1`)
  }
  if (isLoading) {
